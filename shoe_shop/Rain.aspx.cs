...
//            try
//            {
//                da = new SqlDataAdapter("SELECT p.*, cc.CategoryName AS catName FROM product_tbl p JOIN category_tbl cc ON p.CategoryId = cc.Id WHERE cc.CategoryName = 'Rain';", con);
//                ds = new DataSet();
//                da.Fill(ds);
...
//                pg.AllowPaging = true;
//                pg.PageSize = 6; // 6 products per page
    ...
    public partial class Rain : System.Web.UI.Page
    {
        ...
        protected void LinkButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Ensure productId is a valid integer before redirecting
                if (int.TryParse(productId, out int validId))
                {
                    Response.Redirect($"show_product.aspx?ProId={validId}");
                }
                else
                {
        ...
        protected void DataList1_ItemCommand(object source, DataListCommandEventArgs e)
        {
            if (e.CommandName == "EditProduct")
            {
                ...
                // Validate productId before redirecting
                if (int.TryParse(productId, out int validId))
                {
                    Response.Redirect($"show_product.aspx?ProId={validId}");
                }
